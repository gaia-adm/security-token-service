...
validate $? 'SUCCESS: Tenant '$TENANT_NAME' created successfully' 'ERROR: Failed to create tenant '$TENANT_NAME

##### check that tenant created
curl -i -H "Accept: application/json" http://localhost:9001/sts/tenant/$TENANT_NAME  | grep '200 OK'
validate $? 'SUCCESS: Tenant ' $TENANT_NAME' is available' 'ERROR: Failed to get tenant details for '$TENANT_NAME

##### get tenant id for further usage
TENANT_ID=$(curl -H "Accept: application/json" http://localhost:9001/sts/tenant/$TENANT_NAME | grep 'tenantId' | sed s/,/\\n/g | grep 'tenantId' | sed -s 's/"tenantId":\(.*\)/\1/'  | sed -r 's/\{//g')
validate $? 'SUCCESS: Successfully got tenant data for '$TENANT_NAME', its id is '$TENANT_ID 'ERROR: Cannot get tenant id for tenant '$TENANT_NAME
